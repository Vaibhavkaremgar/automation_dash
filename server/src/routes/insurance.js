  ...
  try {
    ...
    if (vertical && vertical !== 'all') {
      if (vertical === 'general') {
        ...
        // 2-wheeler: veh_type contains 2wh/2wheeler OR vertical=2-wheeler
        query += ' AND (LOWER(TRIM(veh_type)) LIKE ? OR LOWER(TRIM(veh_type)) LIKE ? OR LOWER(TRIM(veh_type)) LIKE ? OR vertical = ?)';
        params.push('%2wh%', '%2 wh%', '%2 wheeler%', '2-wheeler');
      } else if (vertical === 'motor') {
        const { generalSubFilter } = req.query;
        if (generalSubFilter === 'motor') {
          // 4-wheeler: veh_type contains 4wh/4wheeler OR (veh_type is empty AND vertical=motor)
          query += ' AND (LOWER(TRIM(veh_type)) LIKE ? OR LOWER(TRIM(veh_type)) LIKE ? OR LOWER(TRIM(veh_type)) LIKE ? OR ((veh_type IS NULL OR TRIM(veh_type) = "") AND vertical = ?))';
          params.push('%4wh%', '%4 wh%', '%4 wheeler%', 'motor');
        } else {
