    ...
    <header className="h-14 border-b border-slate-800 bg-slate-900/60 backdrop-blur flex items-center justify-between px-2 md:px-4 relative z-20">
      ...
      <div className="flex items-center gap-1 md:gap-3 flex-shrink-0">
        <button
          ...
          className="px-2 md:px-3 py-1.5 bg-indigo-900/20 border border-indigo-700/50 rounded-lg text-indigo-300 text-sm hover:bg-indigo-900/40 transition-all flex items-center gap-1 md:gap-2"
        >
          <span className="w-6 h-6 rounded-full bg-indigo-500 flex items-center justify-center text-xs font-bold text-white flex-shrink-0">
            {profileName ? profileName.charAt(0).toUpperCase() : '👤'}
          </span>
          <span className="hidden md:inline">{profileName || 'Profile'}</span>
        </button>
        
        {isInsuranceClient && (
          <select
            className="px-2 md:px-3 py-1.5 bg-slate-800/60 border border-slate-700 rounded-lg text-white text-xs md:text-sm focus:outline-none focus:border-indigo-500"
          ...
          >
            ...
            <option value="non-motor">🏠 Non-M</option>
        ...
        <motion.button
          onClick={logout}
          className="px-2 md:px-3 py-2 bg-red-900/20 hover:bg-red-900/40 border border-red-700/50 hover:border-red-600 rounded-lg text-red-400 hover:text-red-300 text-sm font-medium transition-all duration-200 flex items-center gap-1 md:gap-2"
