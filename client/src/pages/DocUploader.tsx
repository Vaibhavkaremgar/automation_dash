...
export default function DocUploader() {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'embedded'>('dashboard')

  const DOC_UPLOADER_URL = 'https://document-system-production-1a7e.up.railway.app/'

  useEffect(() => {
    fetchDocActivity()
    const interval = setInterval(fetchDocActivity, 30000)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-6">
      <div className="max-w-6xl mx-auto">
          ...
          <>
            ...
            <div className="mt-8 p-4 bg-indigo-500/10 border border-indigo-500/30 rounded-lg">
              <p className="text-indigo-300 text-sm">
                <strong>💡 Tip:</strong> Use the "Doc Uploader" tab to access the full interface, or click "Open Doc Uploader" to open in a new window. Your recent activity will sync automatically every 30 seconds.
              </p>
            </div>
          </>
        )}
        ...
        {/* Embedded Tab */}
        {activeTab === 'embedded' && (
          <div className="bg-slate-800/50 border border-slate-700 rounded-lg overflow-hidden" style={{ height: 'calc(100vh - 300px)' }}>
            <iframe
