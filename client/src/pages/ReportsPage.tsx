...
export default function ReportsPage() {
  ...
  const loadReports = async () => {
    try {
      ...
      // Override premium collection with Dashboard logic
      if (response.data && response.data.premiumCollection && response.data.premiumCollection.customers) {
        const customers = response.data.premiumCollection.customers;
        const now = new Date();
        
        // This Month Premium - same logic as Dashboard
        ...
        // This Year Premium - same logic as Dashboard
        const thisYear = customers.filter((c: any) => {
          const status = c.status?.trim().toLowerCase().replace(/[\s-]/g, '');
          if (status !== 'renewed' && status !== 'inprocess' && status !== 'inprogress') return false;
          const dateStr = (c.renewal_date?.trim() || c.od_expiry_date?.trim());
          if (!dateStr) return false;
        ...
        response.data.premiumCollection.collectedThisMonth = thisMonth.reduce((sum: number, c: any) => sum + parseAmount(c.premium), 0);
        response.data.premiumCollection.collectedThisYear = thisYear.reduce((sum: number, c: any) => sum + parseAmount(c.premium), 0);
      }
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {activeTab === 'premium' && (() => {
        ...
        const now = new Date();
        const collectedThisMonth = (() => { const now = new Date(); const thisMonth = reportData.premiumCollection.customers.filter(c => { const status = c.status.trim().toLowerCase().replace(/[\s-]/g, ''); if (status !== 'renewed' && status !== 'inprocess' && status !== 'inprogress') return false; const dateStr = (c.renewal_date?.trim() || c.od_expiry_date?.trim()); if (!dateStr) return false; try { const [d, m, y] = dateStr.split('/'); const date = new Date(parseInt(y), parseInt(m) - 1, parseInt(d)); return date.getMonth() === now.getMonth() && date.getFullYear() === now.getFullYear(); } catch (e) { return false; } }); return thisMonth.reduce((sum, c) => sum + (parseFloat(c.premium) || 0), 0); })();
        const collectedThisYear = (() => { const now = new Date(); const thisYear = reportData.premiumCollection.customers.filter(c => { const status = c.status.trim().toLowerCase().replace(/[\s-]/g, ''); if (status !== 'renewed' && status !== 'inprocess' && status !== 'inprogress') return false; const dateStr = (c.renewal_date?.trim() || c.od_expiry_date?.trim()); if (!dateStr) return false; try { const [d, m, y] = dateStr.split('/'); const date = new Date(parseInt(y), parseInt(m) - 1, parseInt(d)); return date.getFullYear() === now.getFullYear(); } catch (e) { return false; } }); return thisYear.reduce((sum, c) => sum + (parseFloat(c.premium) || 0), 0); })();
